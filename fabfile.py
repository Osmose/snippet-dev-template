...
def monitor_build_push():
    ...
    kq.control([ke], 0, None)
    print "Monitoring for changes..."

    while True:
        raised_events = kq.control([ke], 1, None)
        file_change = False
        for event in raised_events:
            if event.fflags & (select.KQ_NOTE_DELETE | select.KQ_NOTE_WRITE):
                file_change = True
                break
...
def db_setup():
    ...
    db_type = raw_input('Select the type of database your snippets server uses '
                        '(sqlite3, or blank to skip): ')
    config.set('Database', 'db_type', db_type)

    if db_type == 'sqlite3':
        db_path = raw_input('Enter the absolute path to the database file: ')
...
def build_snippet():
    """
    Combines the compiled JS, CSS, and content into the template and outputs the
    result.
    """
